...
阶段 1 广播   各 AP 播报自身实测数据（TX Power、RSSI、信道占用等）
    ↓
阶段 2 提案   状况最差的 AP 调用计算工具，生成参数调整方案
    ↓
阶段 3 投票   其余 AP 调用验算工具，验证参数约束后表态
    ↓
...
每个 AP Agent 拥有 4 个可调用工具（定义在 `src/tools/registry.py`，说明见各 `agents/ap*/TOOLS.md`）：

| 工具名 | 阶段 | 作用 |
|--------|------|------|
| `compute_sr_recommendations` | 提案 | 扫描满足 CCA/SINR/STA-RSSI 三重约束的推荐发射功率 |
| `compute_edca_recommendations` | 提案 | 根据拥塞等级推荐 CWmin/CWmax/AIFSN |
| `validate_sr_proposal` | 投票 | 验算提案功率是否违反物理约束 |
| `validate_edca_proposal` | 投票 | 验算提案 EDCA 参数是否在合法范围内 |
