...
本 agent 不参与 `run_openclaw.py` 的默认路径，只在显式 `--use-coordinator` 或手工调用 coordinator 时启用。
...
1. 广播阶段：ap1/ap2/ap3 并发生成，按 ap1 → ap2 → ap3 顺序记录和展示。
2. 提案阶段：默认 ap1 发起首个提案，AP 自主选择 Co-SR / Co-EDCA / 联合 / 暂不调整。
3. 投票阶段：非提案 AP 连续验算并投票；反对者可带反提案接管。
4. 决策阶段：全票通过后系统直接采用已通过的提案 JSON，不再调用提案方；随后由 Validator 确定性验收。
