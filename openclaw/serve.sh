#!/usr/bin/env bash
# 绑定启动 Multi-AP 的后台长期服务：state server(ns3/ap only) + 常驻 multiap gateway
...
# academic plot：matplotlib GUI 窗口（ns3/真实通用可视化）。无桌面时 start 跳过。
...
start_state() {
    ...
    echo "[serve] 启动 state server（仅接收 source=ns3/ap）..."
