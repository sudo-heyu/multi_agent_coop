...
# 真实 AP/hostapd 上报的 cwmin/cwmax 是指数 n；协商内部统一使用实际 CW 值推理
# （范围 [3,1023]）；下发硬件时再转回指数。ns-3 遥测输出的已经是实际 CW 值，
# 不能再解码。AIFSN 本身是直接计数值，不参与此转换。
...
def decode_state_edca(state: dict) -> dict:
    """把真实 AP 状态里上报的 cwmin/cwmax 指数解码为实际 CW 值（返回新字典）。

    source="ns3" 的遥测已经是实际 CW 值，原样保留。source="ap" 或缺省时按
    hostapd/iw 的指数 n 表示转换为协商内部使用的实际 CW 值。
    其余字段原样保留；非数值的 cwmin/cwmax 跳过。
    """
    if not isinstance(state, dict):
        return state
    out = dict(state)
    if str(out.get("source", "")).lower() == "ns3":
        return out
